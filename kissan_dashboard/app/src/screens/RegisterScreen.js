...
export default function RegisterScreen({ navigation }) {
    ...
    try {
      ...
      if (data.success) {
        ...
        navigation.replace('MainApp', {
          kisan: loginData.success ? loginData.kisan : { full_name: fullName.trim(), id: data.kisanId },
        });
      } else {
        Alert.alert('Error', data.error);
      }
  ...
  const renderStep1 = () => (
    <View style={styles.card}>
      ...
      <Text style={styles.cardTitle}>🌾 Create Kisan ID</Text>
      ...
      <View style={styles.buttonRow}>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.primaryBtnText}>🌾 Register Kisan ID</Text>
          )}
        </TouchableOpacity>
      </View>
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView ref={scrollRef} contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>किसान पंजीकरण</Text>
          <Text style={styles.headerSub}>Kisan Registration</Text>
          {/* Progress */}
